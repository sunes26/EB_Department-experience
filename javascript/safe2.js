...
const choice_btn = document.getElementsByClassName("choice_btn");
...
for(let i = 0; i < choice_btn.length; i++){
  choice_btn[i].addEventListener("click", function(){
    for(let j=0;j<choice_btn.length;j++){
      choice_btn[j].style.background = "#D9D9D9";
      choice_btn[j].style.color="black";
    }
    choice_btn[i].style.background = "#5D5FF6";
    choice_btn[i].style.color="white";
    submit_btn.style.background = "#5D5FF6";
    submit_btn.style.color="white";
   choice_value = choice_btn[i].value;
